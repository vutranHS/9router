 ...
 */
export async function handleForcedSSEToJson({ providerResponse, sourceFormat, targetFormat, provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, onQuotaSettled, customToolNames, trackDone, appendLog, reqTag, log }) {
  ...
  if (isCodexResponsesApi) {
    try {
      ...
      if (onQuotaSettled) await Promise.resolve().then(() => onQuotaSettled(usage)).catch((error) => console.error("[Quota] Settlement failed:", error.message));
