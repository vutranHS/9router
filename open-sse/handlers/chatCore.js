...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onQuotaSettled, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, headroomTimeoutMs, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking }) {
  ...
  const streamController = createStreamController({
    onDisconnect: (reason) => {
      ...
      if (onQuotaSettled) Promise.resolve().then(() => onQuotaSettled(null)).catch((error) => console.error("[Quota] Disconnect settlement failed:", error.message));
      if (onDisconnect) onDisconnect(reason);
    },
    onError: () => {
      trackPendingRequest(model, provider, connectionId, false);
      if (onQuotaSettled) Promise.resolve().then(() => onQuotaSettled(null)).catch((error) => console.error("[Quota] Error settlement failed:", error.message));
    },
  ...
  const sharedCtx = { provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, onQuotaSettled, pxpipe: pxpipeSummary, reqTag, log };
