...
export function buildUsage({ promptTokens, completionTokens, totalTokens, cachedTokens = 0, cacheCreationTokens = 0, cacheCreation, reasoningTokens = 0 }) {
  ...
  if (cachedTokens > 0 || cacheCreationTokens > 0) {
    ...
    if (cacheCreation) usage.prompt_tokens_details.cache_creation = cacheCreation;
  }
...
const USAGE_EXTRACTORS = {
  claude(raw) {
    ...
    return { promptTokens: prompt, completionTokens: output, totalTokens: prompt + output, cachedTokens: cacheRead, cacheCreationTokens: cacheCreate, cacheCreation: raw.cache_creation };
  },
  gemini(raw) {
