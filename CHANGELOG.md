...
## Fixes
- **HUD quota**: Codex 5h/7d windows no longer show `--`. Some Codex usage responses omit `limit_window_seconds`, so `normalizeQuota` dropped both windows under strict duration matching (`=== 18000` / `=== 604800`). Windows are now mapped by role — the non-weekly primary is the 5h window, the secondary is the 7d window — accepting the standard duration or an absent one, while a *defined* non-standard duration is still rejected rather than mislabeled
